...
class DemandDB:
    ...
    def get_all_current_levels(self):
        res = self.influx.query('select sum(quantity) from demand group by bean')
    ...
    def get_current_level(self, bean):
        res = self.influx.query('select sum(quantity) from demand where bean = $bean', bind_params={'bean': bean})
        try:
